...
npm run deploy:local
  ├─ next build (本地构建)
  ├─ tar 打包 standalone (~3.5MB, --exclude 开发文件)
  ├─ scp 上传 → /tmp/deploy-artifact.tar.gz
  └─ ssh 触发 deploy-remote.sh          deploy-remote.sh
                                          ├─ flock 加锁
                                          ├─ 校验 tarball
                                          ├─ 原子替换 standalone/
                                          ├─ pm2 delete + start (重建主应用)
                                          ├─ pm2 delete + start (重建 webhook)
                                          ├─ 健康检查
                                          └─ 清理（保留本地 tar 包供下次覆盖）
...
| 上传数据量     | 无                    | ~3.5MB（standalone tarball，排除开发文件） |
...
Phase 3  打包
  ├─ tar czf deploy-artifact.tar.gz --exclude 开发文件 -C .next/standalone .
  └─ 排除: src/, scripts/, doc/, supabase/, 配置文件, 文档等（运行时不需要）

Phase 4  上传 + 校验
  ├─ scp deploy-artifact.tar.gz ewing.top:/tmp/
  ├─ ssh sync（强制文件系统刷盘，见下方"偶发 server.js 缺失"）
  └─ 远端文件大小 + tar 完整性校验（最多重试 3 次）

Phase 5  远端部署
  └─ ssh ewing.top "bash .../deploy-remote.sh"（实时输出）

Phase 6  输出耗时（本地 tarball 保留，下次部署自动覆盖）
```
...
Phase 5  重建应用
  ...
  ├─ pm2 delete vibe_blog_next → pm2 start（重建主应用）
  ├─ pm2 delete webhook → pm2 start（重建 webhook，确保脚本变更生效）
  └─ pm2 save
...
- **服务端 localhost 检查**（deploy-remote.sh）：使用 `http://localhost:$PORT/api/healthz`，严格验证 commit hash 是否与构建一致。验证失败自动回滚。
- **部署机公网可达性检查**（deploy-local.mjs）：远端部署完成后，从部署机发起 `https://<SSH_HOST>/api/healthz` 请求，验证公网可达。若公网不可达，回退到 SSH 到服务器再次 localhost 检查。
...
| `NEXT_PUBLIC_BUILD_HOST`         | `COMPUTERNAME` 环境变量（构建机器名） |
...
**现象：** `npm run deploy:local` 偶发报错 `❌ 构建产物缺少 server.js (共 xxxx 个条目)`，多试几次又能成功。

**根因：** Linux 服务器的 `/tmp` 可能挂载为 tmpfs（内存文件系统）。SCP 传输完成后，操作系统尚未将数据完全刷盘。`deploy-remote.sh` 立即执行 `tar tzf` 读取到的是不完整的文件，导致 `server.js` 不存在于列表中。重试时文件系统已完成刷盘，故能成功。

**修复：** `deploy-local.mjs` 上传后会先执行远端 `sync` 强制刷盘，再进行文件大小比对 + `tar tzf` 完整性校验，最多重试 3 次（每次间隔 2 秒），全部失败才报错退出。

**如果仍频繁失败：**

```bash
# 检查服务器 /tmp 是否为 tmpfs
ssh ewing.top "df -h /tmp && mount | grep tmp"

# 检查服务器内存是否充足（tmpfs 依赖内存）
ssh ewing.top "free -h"
```
...
> **注意：** 每次 `npm run deploy:local` 部署时会自动重建 webhook 服务（delete + start），无需手动重启。但依赖变更仍需手动 `npm ci`。
