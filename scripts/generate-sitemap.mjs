...
function extractTitle(filePath) {
  try {
    ...
    const match = content.match(/export\s+const\s+metadata[^}]*?title:\s*['"](.+?)['"]/s)
...
for (const r of routes) {
  ...
  lines.push(`  { path: '${r.path}', title: '${title.replace(/'/g, "\\'")}' },`)
}
...
console.log(`✓ 已生成 sitemap 路由数据: ${routes.length} 条路由 → ${relative(process.cwd(), OUTPUT)}`)
