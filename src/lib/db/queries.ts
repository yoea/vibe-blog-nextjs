...
export async function getPublishedPosts(page = 1, limit = 10) {
  ...
  const authorMap = new Map(
    (userSettings ?? []).map((s) => [
      s.user_id,
      { name: s.display_name, avatar_url: s.avatar_url ?? null },
    ]),
  );
...
export async function getCommentsForPost(
  ...
  const { data: topLevelComments, error } = results[2] ?? {
    data: null,
    error: null,
  };
  ...
  const authorIds = [
    ...new Set(allComments.map((c) => c.author_id).filter(Boolean)),
  ];
    ...
    (async () => {
      ...
      if (likes) {
        for (const l of likes) {
          likeCountMap.set(
            l.comment_id,
            (likeCountMap.get(l.comment_id) ?? 0) + 1,
          );
...
export async function getGuestbookMessages(
  ...
  const authorIds = [
    ...new Set(allMessages.map((m) => m.author_id).filter(Boolean)),
  ];
  const settingsMap = new Map<
    string,
    { display_name: string | null; avatar_url: string | null }
...
export async function getAllUsers(page = 1, limit = 20) {
  ...
  const { data: settings, error } = await supabase
    .from('user_settings')
    .select(
      'user_id, display_name, avatar_url, github_id, github_username, is_deleted, deleted_at, created_at',
    )
  ...
  return {
    data: (settings ?? []).map((s) => ({
      id: s.user_id,
      displayName:
        s.display_name || emailMap.get(s.user_id) || s.user_id.slice(0, 8),
...
export async function getAllTags(): Promise<
  { name: string; slug: string; color: string | null; post_count: number }[]
> {
  const supabase = await createClient();
  ...
  try {
    ...
    if (tagError || !tag) {
      return {
        ...
        tagName: '',
        tagColor: '',
        error: `标签查询失败: ${tagError?.message ?? '标签不存在'}`,
      };
    }
    ...
    const result = posts.map((item: any) => ({
      ...
      author: {
        name:
          authorMap.get(item.author_id)?.name ?? item.author_id?.slice(0, 8),
        avatar_url: authorMap.get(item.author_id)?.avatar_url ?? null,
      },
      like_count: item.like_count?.[0]?.count ?? 0,
      comment_count: item.comment_count?.[0]?.count ?? 0,
