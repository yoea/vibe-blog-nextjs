...
function checkRateLimit(): { allowed: boolean; message?: string } {
  ...
  if (recentClicks.length >= MAX_CLICKS_PER_HOUR) {
    return { allowed: false, message: '感谢你的喜爱！今天已经点赞够了，明天再来吧 😊' }
  }
...
export function SiteStats({ initialViews, initialLikes }: { initialViews: number; initialLikes: number }) {
  ...
  return (
    <div className="flex items-center justify-center gap-4 text-muted-foreground select-none">
      ...
      >
        <Heart className={`h-3.5 w-3.5 transition-all duration-150 ${isLiking ? 'scale-125 fill-pink-400 text-pink-400' : 'fill-pink-400 text-pink-400'}`} />
        <span>{likes}</span>
      </button>
