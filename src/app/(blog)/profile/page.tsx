...
export default async function ProfilePage() {
  ...
  const {
    ...
    count,
    error: postsError,
  } = await getPostsByAuthor(user.id, 1, 10);

  // Fetch guestbook messages left for this user
  const { data: guestbookMessages, total: guestbookTotal } =
    await getGuestbookMessages(user.id, { page: 1, pageSize: 10 });
  ...
  const githubIdentity =
    user.identities?.find((i) => i.provider === 'github') ?? null;
  ...
  const githubUsername = githubIdentity
    ...
    : null;
  const isGitHubUser =
    user.app_metadata?.provider === 'github' || !!githubIdentity;
